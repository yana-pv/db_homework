...
b. 2 партиции 
Участвующие секции:
- client_order_2026_01 (диапазон: 2026-01-01 до 2026-02-01)
- client_order_2026_02 (диапазон: 2026-02-01 до 2026-03-01)
...
b. 1 партиция
Участвующая секция:
- client_order_completed (хранит заказы со статусом 'выполнен')
...
b. 1 партиция
Участвующая секция:
- client_order_hash_0 (где MODULUS 4, REMAINDER 0)
c. Индекс ИСПОЛЬЗУЕТСЯ
- Bitmap Index Scan on client_order_hash_0_id_client_idx — используется индекс
- Bitmap Heap Scan — чтение таблицы по битовой карте от индекса
- Index Cond: (client_order_hash.id_client = 12345)
...
На REPLICA1 (физическая реплика, порт 5433):
```
SELECT * FROM pg_class WHERE relname LIKE 'test_partition%';
```
![Скриншот](img5/4.png)
![Скриншот](img5/5.png)
...
### Создаем подписку на публикацию с publish_via_partition_root = false на логической реплике:
```
DROP TABLE IF EXISTS test_partition_2026_01 CASCADE;
DROP TABLE IF EXISTS test_partition_2026_02 CASCADE;
DROP TABLE IF EXISTS test_partition_2026_03 CASCADE;
DROP TABLE IF EXISTS test_partition_default CASCADE;

CREATE TABLE test_partition_2026_01 (
  id INTEGER,
  created_date DATE,
  data TEXT,
  region VARCHAR(50),
...
CREATE TABLE test_partition_2026_02 (
  id INTEGER,
  created_date DATE,
  data TEXT,
  region VARCHAR(50),
...
CREATE TABLE test_partition_2026_03 (
  id INTEGER,
  created_date DATE,
  data TEXT,
  region VARCHAR(50),
...
CREATE TABLE test_partition_default (
  id INTEGER,
  created_date DATE,
  data TEXT,
  region VARCHAR(50),
...
-- Создаем подписку на публикацию с publish_via_partition_root = false
DROP SUBSCRIPTION IF EXISTS test_partition_sub_off;

CREATE SUBSCRIPTION test_partition_sub_off
CONNECTION 'host=postgres port=5432 dbname=autoservice user=admin password=admin'
PUBLICATION test_partition_pub_off
WITH (copy_data = true, create_slot = true);
```

### Создаем подписку на публикацию с publish_via_partition_root = true на логической реплике:
```
...
-- Создаем подписку
DROP SUBSCRIPTION IF EXISTS test_partition_sub_on;

CREATE SUBSCRIPTION test_partition_sub_on
CONNECTION 'host=postgres port=5432 dbname=autoservice user=admin password=admin'
PUBLICATION test_partition_pub_on
WITH (copy_data = true, create_slot = true);
```

### Проверка результатов на реплике
```
-- На подписчике для OFF режима
SELECT 'test_partition_2026_01' as table_name, COUNT(*) FROM test_partition_2026_01
UNION ALL
SELECT 'test_partition_2026_02', COUNT(*) FROM test_partition_2026_02
UNION ALL
SELECT 'test_partition_2026_03', COUNT(*) FROM test_partition_2026_03
UNION ALL
SELECT 'test_partition_default', COUNT(*) FROM test_partition_default;
```
![Скриншот](img5/9.png)


```
-- На подписчике для ON режима (все данные в одной таблице)
SELECT COUNT(*) as total_rows FROM test_partition_root;
```
![Скриншот](img5/10.png)
...
```
![Скриншот](img5/12.png)

![Скриншот](img5/13.png)

### Запросы и анализ планов
```
-- Запрос на все данные 
EXPLAIN (ANALYZE, BUFFERS)
SELECT COUNT(*), SUM(total_amount) FROM shard_router;
```
![Скриншот](img5/14.png)
